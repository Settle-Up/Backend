...
public interface ReceiptRepository extends JpaRepository<ReceiptEntity,Long> {
    ...
    @Modifying
    @Query("DELETE FROM ReceiptEntity r WHERE r.payerUser.id = :userId")
    void deleteByPayerUserId(@Param("userId") Long userId);

    @Modifying
    @Query("DELETE FROM ReceiptEntity r WHERE r.group.id = :groupId")
